...
export default {
  ...
  data() {
    return {
      ...
      filters: [
        { name: "Name",field:"journalist_name",select:"" },
        { name: "Title",field:"journalist_contact_title",select:"" },
        { name: "Country",field:"journalist_contact_country",select:"" },
        { name: "Subjects",field:"journalist_contact_subjects",select:"" },
      ],
      topic_select: {
        select: '',
        items: [""],
  ...
  methods: {
    ...
    getStatus() {
      ...
      }&facet=topic_set&facet=journalist_cnt&facet=hashtags&facet=screen_names&facet=user_mentions&facet=urls&list=journalist&created_at__gte=${
      ...
      axios.get(url_query).then((res) => {
        ...
        this.statisticsData[0].total =
          res.data.facets._filter_journalist_cnt.doc_count;
        this.statisticsData[1].total =
          res.data.facets._filter_journalist_name.journalist_name.buckets.length;
        ...
        this.topic_select.items =
         ...
         this.filters[0].items = 
          res.data.facets._filter_journalist_name.journalist_name.buckets.map(
            (x) => x.key
        );
        this.filters[1].items = 
          res.data.facets._filter_journalist_contact_title.journalist_contact_title.buckets.map(
            (x) => x.key
        );
        this.filters[2].items = 
          res.data.facets._filter_journalist_contact_country.journalist_contact_country.buckets.map(
            (x) => x.key
        );
        this.filters[3].items = 
          res.data.facets._filter_journalist_contact_subjects.journalist_contact_subjects.buckets.map(
            (x) => x.key
        );
