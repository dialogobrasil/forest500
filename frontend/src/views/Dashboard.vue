<template>
  <section>
    ...
    <v-card class="ma-5 d-flex align-center">
      ...
      <v-dialog v-model="dialog" persistent max-width="600px">
        ...
        <v-card height="100%">
          <v-card-title>
            <span class="text-h5">Company Filters</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
 ...
 <vue-datepicker-local v-model="range" :local="local" ></vue-datepicker-local>
    ...
    <v-row class="ma-6">
      ...
      <v-sheet class="ma-5" rounded elevation="3">
        <h1 class="ma-5 pt-5">Top Retweets</h1>
        <v-data-table
...
export default {
  ...
  data() {
    return {
      ...
      filters: [
        { name: "Name", field: "company_name", select: "" },
        { name: "Sectors", field: "company_sectors", select: "" },
        { name: "Headquarters", field: "company_headquarters", select: "" },
        { name: "Supply Chain", field: "company_segments", select: "" },
        { name: "Commodities", field: "company_commodity", select: "" },
        {
          name: "Sustainability commitments",
          field: "company_signatories",
          select: null,
        },
      ],
      topic_select: {
  ...
  methods: {
    ...
    getStatus() {
      ...
      }&facet=topic_set&facet=company_cnt&facet=hashtags&facet=screen_names&facet=user_mentions&facet=urls&list=company&created_at__gte=${
      ...
      axios.get(url_query).then((res) => {
        ...
        this.filters[0].items =
          res.data.facets._filter_company_name.company_name.buckets.map(
            (x) => x.key
          );
        this.filters[1].items =
          res.data.facets._filter_company_sectors.company_sectors.buckets.map(
            (x) => x.key
          );
        this.filters[2].items =
          res.data.facets._filter_company_headquarters.company_headquarters.buckets.map(
            (x) => x.key
          );
        this.filters[3].items =
          res.data.facets._filter_company_segments.company_segments.buckets.map(
            (x) => x.key
          );
        this.filters[4].items =
          res.data.facets._filter_company_commodity.company_commodity.buckets.map(
            (x) => x.key
          );
        this.filters[5].items =
          res.data.facets._filter_company_signatories.company_signatories.buckets.map(
            (x) => x.key
          );
