...
class LatestStatus(APIView):
    def get(self, request, format=None):
        ...
        topic = request.GET.get('topic', None)
        source_panel = request.GET.get('source_panel', None)
        created_at_gt = datetime.datetime.strptime((request.GET.get('created_at_gte', None)),'%Y-%m-%d')
        created_at_lt = datetime.datetime.strptime((request.GET.get('created_at_lte', None)),'%Y-%m-%d')
        

        status = StatusDocument.search().filter('term',topic_set__name__raw=topic).filter('range', created_at={'gte': created_at_gt, 'lte': created_at_lt}).filter('exists',field='user_mentions.screen_name').filter('term',user__handle_set__handlerel_set__list__raw=source_panel).scan()
        ...
        for x in status:
            ...
            targets =  [y.screen_name for y in x.user_mentions]
